---
name: tfl
description: London TfL transit — real-time Tube arrivals, bus predictions, line status, service disruptions, journey planning, route info, and scheduled timetable lookups for first/last services between stations on supported TfL rail lines. Use when the user asks about London public transport, Tube times, bus arrivals, TfL service status, or scheduled first/last services.
...
Real-time London TfL transit data — Tube arrivals, bus predictions, line status, disruptions, journey planning, route information, and scheduled timetable lookups for first/last services between stations. Uses TfL's Unified API for live data and timetable lookups for scheduled rail services. API key optional (free, recommended for higher rate limits).
...
- User asks about the last train / final Tube / first or last service between two stations
...
For **scheduled timetable lookups**, this skill uses TfL's timetable endpoint: `GET /Line/{lineId}/Timetable/{fromStopPointId}/to/{toStopPointId}`. The timetable payload returns schedule groups such as Monday - Thursday, Friday, Saturday (also Good Friday), and Sunday, plus the journey intervals needed to compute first/last services that actually reach the destination stop.
...
- `timetable` uses TfL's Unified API timetable endpoint, so it works for station pairs on TfL rail lines where timetable data is exposed through that API
...
| `api.tfl.gov.uk/Line/{lineId}/Timetable/{fromStopPointId}/to/{toStopPointId}` | API key (query param, optional) | Scheduled line timetable JSON for pair first/last service lookup |
