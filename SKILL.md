---
name: tfl
description: London TfL transit — real-time Tube arrivals, bus predictions, line status, service disruptions, journey planning, route info, and scheduled last-train lookups for the London Underground, DLR, Overground, Elizabeth line, and buses. Use when the user asks about London public transport, Tube times, bus arrivals, TfL service status, or last train times.
...
Real-time London TfL transit data — Tube arrivals, bus predictions, line status, disruptions, journey planning, route information, and scheduled last-train lookups. Uses TfL's Unified API for live data and line timetable lookups. API key optional (free, recommended for higher rate limits).
...
- User asks about the last train / final Tube / first or last service on a Tube line
...
For **scheduled first/last service lookups**, this skill uses TfL's timetable endpoint on the Unified API: `GET /Line/{lineId}/Timetable/{fromStopPointId}/to/{toStopPointId}`. That returns schedule groups such as Monday - Thursday, Friday, Saturday (also Good Friday), and Sunday, plus the journey intervals needed to compute the final service that actually reaches the destination station.
...
- `last-train` uses TfL's Unified API timetable endpoint, so it currently supports Tube lines with timetable data exposed through that API
- `--day` on `last-train` accepts `tonight`, `monday-thursday`, `friday`, `saturday`, or `sunday`
- `last-train --json` returns a compact payload by default; add `--all` to include the full matching timetable in JSON
...
| `api.tfl.gov.uk/Line/{lineId}/Timetable/{fromStopPointId}/to/{toStopPointId}` | API key (query param, optional) | Scheduled line timetable JSON for first/last service lookup |
